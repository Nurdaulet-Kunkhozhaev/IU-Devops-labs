## README Lab 14
```
user@nurdaulet k8s % kubectl get po,sts,svc,pvc,cm
...
## Grafana
Dunno why there are issues :shrug:
```
user@nurdaulet k8s % minikube service devops-time-app-grafana                
...
```
```
user@nurdaulet k8s % minikube status
minikube
type: Control Plane
host: Running
kubelet: Running
...
user@nurdaulet k8s % minikube service monitoring-grafana                                    
...
```
 ```
user@nurdaulet k8s % minikube start                                          
...
user@nurdaulet k8s % helm show values prometheus-community/kube-prometheus-stack | grep  adminPassword
...
P.S. Всего лишь хочу С за курс. И пойду дальше заниматься андроидом